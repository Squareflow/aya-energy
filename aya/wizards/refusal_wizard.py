    ...
    def refuse(self):
        for wizard in self:
            ...
            if wizard.order_id.partner_id:
                wizard.order_id.activity_schedule("note.mail_activity_data_reminder", summary='Offer rejected', note= "The offer "+wizard.order_id.name+" for the client "+wizard.order_id.partner_id.name+" has been rejected please modify it.", user_id= wizard.order_id.user_id.id, date_deadline= datetime.datetime.now()+ datetime.timedelta(days=1))
